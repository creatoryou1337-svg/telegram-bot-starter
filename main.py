...
ANSWERS = [
    ...
    # 9
    "Если у вас возникли технические проблемы с работой приложения, опишите ситуацию в сообщении. "
    "По возможности укажите, на каком этапе возникла ошибка, и приложите скриншот. "
    "Специалист поддержки подключится и поможет разобраться."
]

# Основная клавиатура с темами (по 2 кнопки в ряд)
...
# Клавиатура для ответа (только кнопка "Назад")
...
async def handle_callback(callback: types.CallbackQuery):
    ...
    # Если нажали на тему (topic_0, topic_1, ...)
    if data.startswith("topic_"):
        try:
            # Получаем номер темы из callback_data
            topic_index = int(data.split("_")[1])
            
            if 0 <= topic_index < len(TOPICS):
                # Показываем ответ для этой темы
