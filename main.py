...
ANSWERS = [
    ...
    # 9 - Специальный ответ для техподдержки
...
async def handle_callback(callback: types.CallbackQuery):
    ...
    if data.startswith("topic_"):
        ...
        if topic_index == 8:  # Техническая поддержка
            ...
            # Отправляем специальное сообщение, которое Chatwoot может перехватить
            await callback.message.answer("🔴 ОПЕРАТОР")  # Триггер для Chatwoot
...
async def handle_all_messages(message: types.Message):
    ...
    if user_states.get(user_id, True):
        ...
        # В режиме оператора - сообщение уходит "как есть"
        # Chatwoot должен перехватывать все сообщения от бота
        pass  # Сообщение автоматически уйдет в Telegram
